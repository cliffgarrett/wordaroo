...
export async function loadPuzzle(level = 1) {
  ...
  try {
    ...
    // After S.words, grid, etc. are set up and before play starts:
    S.activeColor = nextCapsuleColor();
    S.nextColor   = nextCapsuleColor();
    S.colorMap    = {};  // per-word color map if you want it later
...
// Simple procedural tone generator for selection feedback
const AudioCtx = window.AudioContext || window.webkitAudioContext;
let audioCtx = null;

function playSelectToneProcedural(step = 1) {
  ...
  // Ascending pitch based on selection length
  // Adjust base frequency & multiplier to taste
  const base = 220;  // A3-ish "thup"
  const freq = base * (1 + (step * 0.15));
  osc.frequency.value = freq;

  // Percussion envelope
  const now = audioCtx.currentTime;
  gain.gain.setValueAtTime(0.0, now);
  gain.gain.linearRampToValueAtTime(0.6, now + 0.01);   // quick pop
  gain.gain.exponentialRampToValueAtTime(0.0001, now + 0.22); // fast decay

  // Optional echo (very subtle)
  delay.delayTime.value = 0.08;      // 80ms echo
  feedback.gain.value = 0.25;        // light trail
  ...
  // Blend echo behind the dry signal
  gain.connect(delay);
  delay.connect(audioCtx.destination);

  osc.start(now);
  osc.stop(now + 0.25); // very short, percussive
}
...
function hookInput(gridEl) {
  ...
  function updateSel() {
    ...
    gridEl.querySelectorAll('.cell.sel').forEach(c => {
      c.classList.remove('sel', 'start', 'middle', 'end', 'single');
      c.style.removeProperty('--capsuleColor');
    });

    if (!S.selPath || !S.selPath.length) return;

    const path = S.selPath;
    const n = path.length;

    // Determine which cells get which edge class
    const classify = (idx, i) => {
    ...
    const color = S.activeColor || S.nextColor || nextCapsuleColor();

    path.forEach((id, i) => {
      const el = cellElById(id);
      if (!el) return;
      el.classList.add('sel', classify(id, i));
      el.style.setProperty('--capsuleColor', color);
    });
  }
  ...
  function acceptMove(nextId) {
    ...
    if (S.selPath.length === 0) {
      ...
      playSelectToneProcedural(S.selPath.length);
      return true;
    }
  ...
  function handlePointerUp(pointerId) {
    ...
    if (i >= 0 && !S.found.has(S.words[i].toUpperCase())) {
      ...
      // clearTempCapsules?.();
      // little clear pulse if you want
      S.selPath.forEach(id => {
        const el = cellElById(id);
        el?.classList.add('clearPulse');
        setTimeout(() => el?.classList.remove('clearPulse'), 250);
      });
      clearLiveLetters();
      S.selPath = [];
      dir = null;
